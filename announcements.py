...
def get_announcement(ignore_last_tag: bool = False) -> dict | None:
    ...
    try:
        response = requests.get("https://api.github.com/gists/e2265148018272bd35fe7403c669a9cd", headers=headers)
        if not response.status_code == 200:
            return None

        content = json.loads(response.json().get("files").get("fpc-box.json").get("content"))
